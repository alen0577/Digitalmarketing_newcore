...
def tl_lead_data(request,pk):
    if 'emp_id' in request.session:
        ...
        leadfield_obj = LeadField_Register.objects.filter(field_work_regId=work_reg)
...
def tl_lead_collected_data(request,pk):
    if 'emp_id' in request.session:
        ...
        leads_obj = Leads.objects.filter(lead_work_regId=work_reg,lead_collect_Emp_id=dash_details,lead_add_date=date.today())
        leads_obj_count = Leads.objects.filter(lead_work_regId=work_reg,lead_collect_Emp_id=dash_details,lead_add_date=date.today()).count()
        lead_Details_obj = lead_Details.objects.filter(leadId__in=leads_obj)
